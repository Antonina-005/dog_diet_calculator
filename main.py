...
is_castrated = "нет"
# Задаем вопрос, только если сука не беременна и не кормит
...
# Базовый расчет по возрастным группам
if dog_age < 1.0:
    ...
    # Для пожилых кастрированных собак метаболизм снижен сильнее всего
    if is_castrated == "да":
    ...
    # Взрослые собаки (от 1 до 7 лет) с учетом кастрации
    if is_castrated == "да":
...
# 1. Беременность и лактация перекрывают все диеты
